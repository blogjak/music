...
if ($_POST['albumID'] != "") {
	...
	if ($_FILES['cover']['error'] > 0) $erreur = "Error while loading image, please try again";
	...
	if ( in_array($extension_upload,$extensions_valides) ) {
		...
		if ($resultat) {
			echo "<p style=\"font-family: Arial, Helvetica, sans-serif;font-size:11px;font-weight:bold;color:#666;\">Your cover has been updated. Please click on the close button to go back</div>";	 
		}
	}
	else {
		$erreur = "Please upload a valid image file (jpg, png or gif)";
	}
